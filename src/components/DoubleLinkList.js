...
import DoublyListToolBar from "./DoublyListToolBar"; // New toolbar
import DoubleListModel from "../models/DoubleListModel"; // New model
import "../styles/DoubleLinkList.css";

const initList = new DoubleListModel();
...
export default function DoubleLinkList() {
  ...
  const handleRun = () => {
    ...
    // Additional edge cases for doubly linked lists
    switch (method) {
      case "push":
      ...
      case "traverseBackwards":
        if (initList.length === 0) {
          alert("Cannot traverse an empty list.");
          return;
        }
        console.log("Traversing backwards:");
        initList.traverseBackwards((node) => console.log(node));
        break;
  ...
  return (
    <div className="DoubleLinkList">
      ...
      <DoublyListToolBar
        ...
        handleRun={handleRun}
      />
      <h2 className="DoubleLinkList-header">Doubly Linked List</h2>
